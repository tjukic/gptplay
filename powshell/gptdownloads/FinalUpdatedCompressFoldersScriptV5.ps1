...
# If there are 3 or more existing archives and no defaultAction is provided, prompt the user for a default action or individual prompts
if ($existingArchivesCount -ge 3 -and (-not $defaultAction)) {
    $initialChoices = @(
        [System.Management.Automation.Host.ChoiceDescription]::new("&Single Answer for All", "Provide a single answer for all existing archives.")
        [System.Management.Automation.Host.ChoiceDescription]::new("&Prompt Individually", "Prompt for each individual archive.")
    )

    $initialDecision = $host.ui.PromptForChoice("Multiple Existing Archives Found", "Do you want to choose an action for each archive individually or provide a single answer for all archives?", $initialChoices, 0)

    # If "Single Answer for All" is chosen, then ask for that single answer
    if ($initialDecision -eq 0) {
        $choices = @(
            [System.Management.Automation.Host.ChoiceDescription]::new("&Overwrite All", "Overwrite all existing archives.")
            [System.Management.Automation.Host.ChoiceDescription]::new("&Update All", "Update all existing archives with new/changed files.")
            [System.Management.Automation.Host.ChoiceDescription]::new("&Skip All", "Skip compressing all folders with existing archives.")
        ...
        $decision = $host.ui.PromptForChoice("Choose a default action for all archives:", "Select the action you'd like to apply to all existing archives:", $choices, 0)
        switch ($decision) {
            0 { $defaultAction = "Overwrite" }
            1 { $defaultAction = "Update" }
            2 { $defaultAction = "Skip" }
...
foreach ($folder in $foldersToCompress) {
    ...
    # If "Prompt Individually" was chosen, reset the $defaultAction for each iteration
    if ($decision -eq 4) {
        $defaultAction = $null
    }
