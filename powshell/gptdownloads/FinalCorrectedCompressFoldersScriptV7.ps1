...
# If there are 3 or more existing archives and no defaultAction is provided, prompt the user
if ($existingArchivesCount -ge 3 -and (-not $defaultAction)) {
    $initialChoices = @(
    ...
    $initialDecision = $host.ui.PromptForChoice("Multiple Existing Archives Found", "Do you want to choose an action for each archive individually or provide a single answer for all archives?", $initialChoices, 1)
